...
const overrideArb = fc.record({
  id: fc.integer({ min: 1 }),
  prayer: fc.constantFrom(
    'fajr',
    'dhuhr',
    'asr',
    'maghrib',
    'isha'
  ) as fc.Arbitrary<PrayerName>,
  overrideType: fc.constantFrom('FIXED', 'OFFSET') as fc.Arbitrary<
    'FIXED' | 'OFFSET'
  >,
  value: fc.string(),
  startDate: dateArb,
  ...
  it('returns true iff startDate <= today <= endDate for any override and any today', () => {
    fc.assert(
      fc.property(overrideArb, dateArb, (override, today) => {
        const expected =
          override.startDate <= today && today <= override.endDate;
